class GardenManager:
    ...
    @classmethod
    def display_all_garden(cls):
        frame = "\033[0;32m===\033[0;0m"
        for garden in cls.gardens:
            print(f"\n{frame}{garden.name}{frame}\n")
            for plant in garden.plants:
                plant.display()

    @classmethod
    def daily_update_all(cls):
        for garden in cls.gardens:
            garden.daily_update()

    @classmethod
    def simulator(cls, days: int):
        for i in range(days):
    ...
    def generation_auto(cls, nbr_garden: int, nbr_plants: int):
        ...
        for j in range(0, nbr_garden):
            garden = cls.add_garden(f"\033[0;32m {garden_names[j]}'s "
                                    f"Garden \033[0;0m")
